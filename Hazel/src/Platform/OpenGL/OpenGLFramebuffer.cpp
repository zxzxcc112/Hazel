            ...
            for (uint32_t i = 0; i < m_ColorAttachments.size(); i++)
            {
                ...
                switch (m_ColorAttachmentSpecifications[i].TextureFormat)
                {
                    case FramebufferTextureFormat::RGBA8:
                        Utils::AttachColorTexture(m_ColorAttachments[i], m_FramebufferSpecification.Samples, GL_RGBA8, m_FramebufferSpecification.Width, m_FramebufferSpecification.Height, i);
                        break;
                }
            }
        }
