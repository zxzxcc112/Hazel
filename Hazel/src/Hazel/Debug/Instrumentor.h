...
namespace Hazel
{
	...
	public:
		...
		void WriteProfile(const ProfileResult& profile)
		{
			...
			json << "\"cat\":\"Function\",";
			json << "\"ph\":\"X\",";
			json << "\"ts\":" << profile.Start.count() << ",";
			json << "\"dur\":" << profile.ElapsedTime.count() << ",";
			json << "\"pid\":0,";
			json << "\"tid\":" << profile.ThreadID;
			json << "}";
	...
	namespace InstrumentorUtils
	{
		...
		constexpr auto CleanupOutputString(const char(&expr)[N], const char(&remove)[K])
		{
			...
			while (srcIndex < N)
			{
				...
				while (matchIndex < N - 1 && srcIndex + matchIndex < N - 1 && expr[srcIndex + matchIndex] == remove[matchIndex])
					matchIndex++;
